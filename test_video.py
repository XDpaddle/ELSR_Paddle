...
def test_video(model,  video, upscale_factor):
    lr_video = []
    bicubic_video = []
    vid = []
    sr_video = []
    ...
    with paddle.no_grad():
        t0 = time()
        for lr in lr_video:
            sr_img = paddle.clip(model(lr),0,1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, default="test/video")
    ...
    sr_video, bicubic_video, video, t = test_video(model,  video, upscale_factor=args.scale)
    ...
    bicubic_psnr=bicubic_psnr.numpy()
    avg_psnr=avg_psnr.numpy()
